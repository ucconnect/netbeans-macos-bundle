...
show_help() {
    echo "./install-from-dir.sh [options]"
    echo
...
DEST_PKG_DIR="${INSTALL_DIR}/NetBeans/NetBeans ${NETBEANS_VERSION}.app"
...
if [ -d "${DEST_PKG_DIR}/" ]; then
    ...
    if [ "${FORCE}" -eq 0 ]
    then
        echo "Refusing to overwrite the existing app ${DEST_PKG_DIR} !"
        ...
        echo "Deleting ${DEST_PKG_DIR} ..."

        read -r -p "Are you sure? [y/N] " response
        case "${response}" in [yY][eE][sS]|[yY])
            ${SUDO_COMMAND}rm -r "${DEST_PKG_DIR}"
            ;;
        *)
...
${SUDO_COMMAND}mkdir -p "${DEST_PKG_DIR}/Contents/MacOS"
${SUDO_COMMAND}mkdir -p "${DEST_PKG_DIR}/Contents/Resources"
...
SRC_DIR_LAST=`basename ${SRC_DIR}`
${SUDO_COMMAND}cp ${CP_MV_VERBOSE} -fR "${SRC_DIR}" "${DEST_PKG_DIR}/Contents/Resources/${SRC_DIR_LAST}"
${SUDO_COMMAND}mv ${CP_MV_VERBOSE} "${DEST_PKG_DIR}/Contents/Resources/${SRC_DIR_LAST}" "${DEST_PKG_DIR}/Contents/Resources/NetBeans"

set +e

# if $NETBEANS_SHA512_URI is set, verify the integrity
COMPUTED_SHA512=`shasum -a 512 "${TMPFILE}" |cut -d " " -f 1`
echo "File SHA512 checksum:     ${COMPUTED_SHA512}"
if [ ! -z "${NETBEANS_SHA512}" ]; then
    echo "Expected SHA512 checksum: ${NETBEANS_SHA512}"
    if [ "${NETBEANS_SHA512}" != "${COMPUTED_SHA512}" ]; then
        echo "Cleaning up..."
        rm "${TMPFILE}"
...
cd "${DEST_PKG_DIR}/Contents/MacOS"
${SUDO_COMMAND}chmod ${CP_MV_VERBOSE} +x ../Resources/NetBeans/bin/netbeans
${SUDO_COMMAND}ln -s ../Resources/NetBeans/bin/netbeans
cd
${SUDO_COMMAND}cp ${CP_MV_VERBOSE} "${DEST_PKG_DIR}/Contents/Resources/NetBeans/nb/netbeans.icns" \
                  "${DEST_PKG_DIR}/Contents/Resources/"
